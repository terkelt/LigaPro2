...
type SimPhase = "running" | "paused" | "match_preview" | "press_conference" | "finished";
...
export function CalendarSimulation({ onClose }: Props) {
  ...
  // Build match preview data from current state
  const buildMatchPreview = useCallback((): MatchPreviewData | null => {
    ...
    for (const schedule of gameState.schedules) {
      for (const match of schedule.matches) {
        ...
        const oppPlayers = gameState.players
          .filter((p) => p.teamId === oppId)
          .sort((a, b) => calcOverall(b) - calcOverall(a))
          .slice(0, 5)
          .map((p) => ({ name: p.lastName, pos: p.position, ovr: calcOverall(p) }));
        ...
        return {
          ...
          opponentShort: oppTeam.shortName,
          opponentId: oppTeam.id,
          opponentColors: oppTeam.colors,
          isHome,
  ...
  const tick = useCallback(() => {
    ...
    // Check: is today a match day?
    if (agenda.isMatchDay) {
  ...
  }, [gameState, advanceOneDay, buildMatchPreview]);
