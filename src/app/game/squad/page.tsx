...
import { Search, ArrowUpDown, Heart, Zap, TrendingUp, TrendingDown, Minus, Shield, Star, HelpCircle } from "lucide-react";
      ...
      <DialogContent className="bg-card border-border max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-3">
            ...
            <div>
              <p className="text-lg font-bold">{player.firstName} {player.lastName}</p>
              <p className="text-xs text-muted-foreground">{POSITION_LABELS[player.position]} | {age} Jahre | {player.nationality}</p>
            </div>
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="grid grid-cols-2 gap-4 mt-2">
          ...
          <div className="space-y-1 text-xs">
            ...
            <div className="flex justify-between"><span className="text-muted-foreground">Potenzial</span><span className="text-accent">{player.potential}</span></div>
        ...
        <div className="mt-3 p-3 rounded-lg bg-secondary/20 border border-border/50">
          ...
          <div className="flex items-center justify-between mt-1.5">
            <span className="text-[10px] text-muted-foreground">Potenzial: <span className="text-accent font-bold">{player.potential}</span> (Ø aktuell: {overall})</span>
            <span className="text-[10px] text-muted-foreground">Wachstum: <span className={player.growthRate >= 0.8 ? "text-green-400" : player.growthRate >= 0.5 ? "text-yellow-400" : "text-orange-400"}>{player.growthRate >= 0.8 ? "Hoch" : player.growthRate >= 0.5 ? "Mittel" : "Gering"}</span></span>
          </div>
          {player.trainingBoosts.length > 0 && (
            <div className="mt-2">
              <p className="text-[10px] text-muted-foreground mb-1">Aktive Trainingseffekte:</p>
              <div className="flex flex-wrap gap-1">
                {player.trainingBoosts.map((b, i) => (
                  <span key={i} className={`text-[10px] px-1.5 py-0.5 rounded ${b.value > 0 ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"}`}>
                    {b.value > 0 ? "+" : ""}{b.value} {b.attribute} (noch {b.weeksRemaining} Wo.)
                  </span>
                ))}
              </div>
            </div>
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-muted-foreground ml-1">{player.firstName}</span>
                            {player.suspended && (
        ...
        <TabsContent value="stats" className="mt-4">
          <Card className="bg-card border-border">
            <CardContent className="flex items-center justify-center py-16 text-muted-foreground text-sm">
              Saisonstatistiken werden nach dem ersten Spieltag verfügbar.
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
