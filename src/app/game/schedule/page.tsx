...
export default function SchedulePage() {
  ...
  const matchdayMatches = useMemo(() => {
    if (!schedule) return [];
    return schedule.matches.filter((m) => m.matchday === selectedMatchday);
            ...
            <div>
              {matchdayMatches.map((match) => {
                ...
                return (
                  <div
                    key={match.id}
                    className={`flex items-center gap-3 px-4 py-3 border-b border-border/50 ${isMyMatch ? "bg-primary/5" : "hover:bg-secondary/20"} ${result ? "cursor-pointer" : ""}`}
                    onClick={() => result && router.push(`/game/match/${match.id}`)}
                  >
                    <span className="text-[10px] text-muted-foreground w-10 shrink-0">{match.time}</span>

                    <div className="flex items-center gap-2 flex-1 justify-end">
                      <span className={`text-sm font-medium text-right ${isMyMatch && match.homeTeamId === currentTeamId ? "text-primary" : ""}`}>
                        {home?.name ?? "?"}
                      </span>
                      <TeamLogo
                    ...
                    <div className="flex items-center gap-2 flex-1">
                      <TeamLogo
                        ...
                        size={20}
                      />
                      <span className={`text-sm font-medium ${isMyMatch && match.awayTeamId === currentTeamId ? "text-primary" : ""}`}>
                        {away?.name ?? "?"}
                      </span>
                    </div>

                    <span className="text-[10px] text-muted-foreground truncate max-w-32 shrink-0 hidden md:block">
                      {match.venue}
                    </span>
                  </div>
                );
