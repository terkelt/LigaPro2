...
export function useYouthPlayers() {
  return useGameStore((s) => s.gameState?.youthPlayers ?? EMPTY_ARRAY);
}

export function useSponsors() {
  return useGameStore((s) => s.gameState?.sponsors ?? EMPTY_ARRAY);
}

export function useSponsorOffers() {
  return useGameStore((s) => s.gameState?.sponsorOffers ?? EMPTY_ARRAY);
}

export function useTransferWindowInfo() {
  return useGameStore(
    useShallow((s) => ({
      isOpen: s.gameState?.isTransferWindowOpen ?? false,
      type: s.gameState?.transferWindowType ?? 'summer',
...
export function useGameActions() {
  return useGameStore(
    useShallow((s) => ({
      ...
      counterIncoming: s.counterIncoming,
      answerPressQuestion: s.answerPressQuestion,
      acceptSponsor: s.acceptSponsor,
      declineSponsor: s.declineSponsor,
      cancelSponsor: s.cancelSponsor,
      negotiateSponsor: s.negotiateSponsor,
      promoteYouth: s.promoteYouth,
      releaseYouth: s.releaseYouth,
    }))
  );
}
