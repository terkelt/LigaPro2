...
export function computeDayAgenda(state: GameState): DayAgenda {
  ...
  // ── Find the week's upcoming match ──
    ...
    for (const match of schedule.matches) {
      ...
      if (days >= 0 && days < closestDays) {
        ...
        weekMatch = {
          match,
          daysAway: days,
          opponentName: opp?.name ?? 'Unbekannt',
          opponentId: oppId,
          isHome,
        };
      }
  ...
  if (todayMatch && weekMatch) {
    items.push({
      id: 'match-today',
      type: 'match',
      title: `⚽ Spieltag: ${weekMatch.isHome ? 'Heim' : 'Auswärts'} vs. ${weekMatch.opponentName}`,
      description: `${todayMatch.matchday}. Spieltag — Anstoß ${todayMatch.time ?? '15:30'} Uhr`,
      icon: '⚽',
      requiresAction: false,
      isResolved: false,
      priority: 0,
      link: `/game/match/${todayMatch.id}`,
  ...
  for (let i = 0; i < 7; i++) {
    ...
    for (const schedule of state.schedules) {
      for (const match of schedule.matches) {
        ...
        matchOpponent = state.teams.find((t) => t.id === oppId)?.shortName;
      }
    }
