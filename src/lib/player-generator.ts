...
function playerFromSeed(
  ...
  const rawPot = seed.pot ?? Math.min(99, overall + (age < 23 ? rng.range(5, 15) : rng.range(0, 5)));
  const potential = Math.max(rawPot, overall); // potential must never be below current OVR
 ...
 */
export function generateTraitsForPlayer(player: Player, explicitTraits?: [string, 'bronze' | 'silver' | 'gold'][]): PlayerTrait[] {
...
/**
 * Generate traits for legendary players — they get MORE traits at HIGHER tiers.
 * Diamond: 4-5 gold traits, Gold: 3-4 gold/silver, Silver: 2-3 silver, Bronze: 1-2 silver/bronze
 */
export function generateTraitsForLegend(player: Player, tier: 'diamond' | 'gold' | 'silver' | 'bronze'): PlayerTrait[] {
  const eligible = TRAIT_CATALOG.filter((t) => t.eligiblePositions.includes(player.position));
  if (eligible.length === 0) return [];
  ...
  const maxTraits = tier === 'diamond' ? 5 : tier === 'gold' ? 4 : tier === 'silver' ? 3 : 2;
  const minTier = tier === 'diamond' ? 'gold' : tier === 'gold' ? 'silver' : 'bronze';

  const traits: PlayerTrait[] = [];
  for (const s of scored) {
    if (traits.length >= maxTraits) break;
    // Legends get traits even with lower scores — they're the best
    let t = tierFromScore(s.score);
    // Upgrade tier for legends: at least minTier
    if (minTier === 'gold' && t !== 'gold') t = 'gold';
    else if (minTier === 'silver' && t === 'bronze') t = 'silver';
    traits.push({ traitId: s.traitId, tier: t, acquiredDate: '2025-07-01' });
  }
...
export function generateAllPlayers(teams: Team[]): Player[] {
  ...
  for (const team of teams) {
    const LEAGUE_TIERS: Record<string, number> = {
      'bundesliga': 1, 'zweite-liga': 2, 'dritte-liga': 3,
      'premier-league': 1, 'la-liga': 1, 'serie-a': 1, 'ligue-1': 1,
      'eredivisie': 1, 'primeira-liga': 1, 'belgian-pro-league': 1, 'scottish-premiership': 1,
    };
    const tier = LEAGUE_TIERS[team.league] ?? 1;
...
/**
 * Generate fresh transfer market players matching the given filters.
 * These are NEW virtual players (not from existing teams) to simulate
 * a real transfer market with infinite depth.
 */
export function generateTransferMarketPlayers(opts: {
  count: number;
  ...
  const targetPos = opts.position && opts.position !== 'all' ? opts.position : undefined;
  const positions: Position[] = targetPos ? [targetPos] : ['TW', 'IV', 'LV', 'RV', 'ZDM', 'ZM', 'ZOM', 'LA', 'RA', 'ST'];

  // Generate more candidates than needed, then filter
  const maxAttempts = opts.count * 8;
  for (let attempt = 0; attempt < maxAttempts && results.length < opts.count; attempt++) {
    const pos = rng.pick(positions);
    const age = rng.range(opts.minAge, opts.maxAge);
    const targetOvr = rng.range(opts.minOvr, opts.maxOvr);

    // Nationality filter
    let nat: string;
    if (opts.nationality && opts.nationality !== 'all') {
      nat = opts.nationality;
    } else {
      nat = pickNationality(rng);
    }

    // Foot filter
    let foot: FootPreference;
    if (opts.foot && opts.foot !== 'all') {
      foot = opts.foot as FootPreference;
    } else {
      foot = rng.next() < 0.7 ? 'right' : rng.next() < 0.5 ? 'left' : 'both';
    }

    const { firstName, lastName } = pickName(nat, rng);
    const attributes = generateAttributes(pos, targetOvr, rng);
    nudgeAttributesToOverall(attributes, pos, targetOvr, rng);
    const overall = calculateOverall(attributes, pos);

    // Check OVR bounds after nudging
    if (overall < opts.minOvr || overall > opts.maxOvr) continue;

    const birthYear = 2025 - age;
    const marketValue = calculateMarketValue(age, overall, 1);
    const salary = calculateSalary(marketValue);
    const potential = Math.min(99, overall + (age < 23 ? rng.range(5, 20) : rng.range(0, 5)));
    const playerId = `tm-${Date.now().toString(36)}-${attempt}-${rng.range(1000, 9999)}`;

    // Assign to a random "foreign" team conceptually — use a placeholder teamId
    const teamId = `market-team-${rng.range(1, 200)}`;

    const player: Player = {
      id: playerId,
      firstName, lastName,
      dateOfBirth: `${birthYear}-${String(rng.range(1, 12)).padStart(2, '0')}-${String(rng.range(1, 28)).padStart(2, '0')}`,
      nationality: nat, position: pos, secondaryPositions: [],
      foot,
      height: pos === 'TW' ? rng.range(185, 198) : pos === 'IV' ? rng.range(180, 195) : rng.range(170, 192),
      weight: rng.range(68, 92),
      shirtNumber: rng.range(1, 99),
      teamId,
      contractUntil: `${2025 + rng.range(0, 4)}-06-30`,
      salary, marketValue, attributes,
      condition: rng.range(75, 100), morale: rng.range(60, 90),
      form: rng.range(50, 80), fatigue: rng.range(0, 15),
      matchPractice: rng.range(40, 75),
      injuryProne: rng.range(1, 7),
      ...
      growthRate: age < 22 ? 0.8 + rng.next() * 0.7 : 0.3 + rng.next() * 0.4,
      level: Math.max(1, Math.floor(overall / 3) + (age > 28 ? 5 : 0)),
      xp: rng.range(0, 50), xpToNextLevel: 100,
      trainingBoosts: [], traits: [],
    ...
    // Generate traits
    player.traits = generateTraitsForPlayer(player);
    if (player.traits.length > 0) {
      let multiplier = 1;
      for (const t of player.traits) {
        const def = TRAIT_CATALOG.find((d) => d.id === t.traitId);
    ...
    // Trait filter
    if (opts.traitId && opts.traitId !== 'all') {
      if (!player.traits.some(t => t.traitId === opts.traitId)) continue;
    }
